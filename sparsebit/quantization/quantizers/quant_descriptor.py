...
class QuantDescriptor:
    ...
    def calc_qmin_qmax(self, bit, scheme):
        if scheme in [torch.per_channel_symmetric, torch.per_tensor_symmetric, "per-group-symmetric"]:
            qmin = -(2 ** (bit - 1))
            qmax = 2 ** (bit - 1) - 1
            _type = "int{}".format(bit)
        elif scheme in [torch.per_channel_affine, torch.per_tensor_affine, "per-group-affine"]:
            qmin = 0
            qmax = 2**bit - 1
