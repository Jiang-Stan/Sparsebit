...
class DataCache(object):
    ...
    def get_data_for_calibration(self, granularity: Granularity, reorder=False):
        ...
        if granularity == Granularity.LAYERWISE:
            ...
            data = torch.cat([d.reshape(d.shape[0], -1) for d in self._data_cache], axis=1)
        elif granularity == Granularity.GROUPWISE:
            if self.target == QuantTarget.FEATURE: #feature group on channel dim
                assert self._data_cache[0].shape[0] <= self.group_size or self._data_cache[0].shape[0] % self.group_size == 0, "group size must be divided by channel num! got {} and {} instead".format(self.group_size, self._data_cache[0].shape[0])
                group_num = max(self._data_cache[0].shape[0] // self.group_size, 1)
                if group_num == 1:
                    self.qdesc.set_group_size = self._data_cache[0].shape[0]
                data = torch.cat([d.reshape(group_num, -1) for d in self._data_cache], axis=1)
